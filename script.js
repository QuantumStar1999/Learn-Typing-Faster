...
const SoundManager = {
    ...
    async init() {
        try {
            ...
            await this.initBackgroundAudio();
    ...
    // Initialize background audio with your MP3 file
    async initBackgroundAudio() {
        return new Promise((resolve, reject) => {
            try {
                // Create new audio element
                this.backgroundAudio = new Audio();
                
                // Set source and configuration
                this.backgroundAudio.src = 'keyboard-typing-sound.mp3';
                ...
                const handleCanPlayThrough = () => {
                    ...
                    this.backgroundAudio.removeEventListener('canplaythrough', handleCanPlayThrough);
                    this.backgroundAudio.removeEventListener('error', handleError);
                ...
                const handleError = (e) => {
                    ...
                    console.error('Audio error details:', this.backgroundAudio.error);
                ...
                // Add event listeners
                this.backgroundAudio.addEventListener('canplaythrough', handleCanPlayThrough);
                this.backgroundAudio.addEventListener('error', handleError);
                this.backgroundAudio.addEventListener('play', handlePlay);
                this.backgroundAudio.addEventListener('pause', handlePause);
    ...
    // Setup audio nodes only when needed
    setupAudioNodes() {
    ...
    updateBackgroundAudio() {
        ...
        try {
            const targetVolume = this.settings.backgroundEnabled ? this.settings.backgroundVolume : 0;
            
            if (this.hasAudioNodes && this.gainNode) {
                // Use Web Audio API for smoother volume control
            ...
            console.warn('Error updating background audio:', error);
        }
    },
    ...
    async toggleBackgroundSound() {
        ...
        this.settings.backgroundEnabled = !this.settings.backgroundEnabled;
        
        if (this.settings.backgroundEnabled) {
            await this.startBackgroundSound();
        } else {
        ...
        try {
            ...
            if (this.hasAudioNodes && this.gainNode) {
                ...
                setTimeout(() => {
                    ...
                    this.isPlaying = false;
                    console.log('Background sound stopped with fade out');
                }, fadeOutDuration * 1000);
                
            } else {
                // Immediate stop for fallback
                this.backgroundAudio.pause();
                this.backgroundAudio.currentTime = 0;
                this.isPlaying = false;
    ...
    setBackgroundVolume(value) {
        this.settings.backgroundVolume = value / 100;
        this.updateBackgroundAudio();
        this.saveSettingsToStorage();
        this.updateUI();
    },
    
    // Set background speed (0.5x-2.0x)
    setBackgroundSpeed(value) {
        this.settings.backgroundSpeed = value / 100;
        this.updateBackgroundAudio();
        this.saveSettingsToStorage();
        this.updateUI();
    },
    
    // Visualizer for background sound
    startVisualizer() {
        this.stopVisualizer();
        
        const visualizerContainer = document.querySelector('.bg-sound-visualizer');
    ...
    // Stop background sound when test ends
    stopBackgroundOnTestEnd() {
        if (this.settings.backgroundEnabled && this.isPlaying) {
            this.stopBackgroundSound();
        }
    },
    ...
    loadSettingsFromStorage() {
        ...
        if (savedSettings) {
            try {
                ...
                console.log('Sound settings loaded from storage');
            } catch (error) {
    ...
    updateUI() {
        // ... (keep your existing updateUI code as is) ...
        // This function remains unchanged
    },
    
    // Show temporary status message
    showSoundStatus(message, type = 'info') {
        // ... (keep your existing showSoundStatus code as is) ...
        // This function remains unchanged
    },
    ...
    playMelody(frequencies, durations) {
        ...
        frequencies.forEach((freq, index) => {
            setTimeout(() => {
                this.generateTone(freq, durations[index] * (1 / this.settings.speed), 'sine');
            }, index * 150);
        });
    },
    ...
    applyPreset(preset) {
        switch(preset) {
            case 'quiet':
                ...
                this.settings.backgroundVolume = 0.2;
                this.settings.backgroundSpeed = 1.0;
                break;
            case 'normal':
                this.settings.volume = 0.7;
                this.settings.speed = 1.0;
                this.settings.backgroundVolume = 0.5;
                this.settings.backgroundSpeed = 1.0;
                break;
            case 'fast':
                this.settings.volume = 0.7;
                ...
                this.settings.backgroundVolume = 0.5;
                this.settings.backgroundSpeed = 1.5;
                break;
        }
...
const SoundManager = {
    ...
    async init() {
        try {
            ...
            this.showSoundStatus('Sound system initialized', 'success');
            
        } catch (error) {
            console.error('Sound initialization failed:', error);
            this.showSoundStatus('Sound initialization failed', 'error');
        }
    },
    setupAudioGraph() {
    ...
    async initBackgroundAudio() {
        return new Promise((resolve, reject) => {
            ...
            this.backgroundAudio.addEventListener('error', (e) => {
                console.error('Background audio error:', e);
                this.audioLoaded = false;
                this.showSoundStatus('Failed to load background sound', 'error');
                reject(e);
            });
    ...
    async toggleBackgroundSound() {
        if (!this.audioLoaded) {
            this.showSoundStatus('Audio not loaded yet', 'warning');
            return;
        }
        ...
        this.showSoundStatus(
            `Background sound ${this.settings.backgroundEnabled ? 'enabled' : 'disabled'}`,
    ...
    async startBackgroundSound() {
        ...
        try {
            ...
            // IMPORTANT: Set volume on the audio element itself
            this.backgroundAudio.volume = this.settings.backgroundVolume;
    ...
    // ========== EXISTING SOUND METHODS (Keep these) ==========
    // Load settings from localStorage
    loadSettingsFromStorage() {
        const savedSettings = localStorage.getItem('typingTestSoundSettings');
        if (savedSettings) {
    ...
    updateUI() {
        ...
        // Update main sound toggle
        if (soundToggle) {
            soundToggle.innerHTML = this.settings.enabled ? 
                '<i class="fas fa-volume-up"></i>' : 
                '<i class="fas fa-volume-mute"></i>';
            soundToggle.classList.toggle('muted', !this.settings.enabled);
        }
        ...
        if (volumeSlider && volumeValue) {
            ...
            volumeSlider.disabled = !this.settings.enabled;
        }
        ...
        if (speedSlider && speedValue) {
            ...
            speedSlider.disabled = !this.settings.enabled;
        }
        
        // Update background sound controls
        if (bgVolumeSlider && bgVolumeValue) {
            const bgVolumePercent = Math.round(this.settings.backgroundVolume * 100);
            bgVolumeSlider.value = bgVolumePercent;
            bgVolumeValue.textContent = `${bgVolumePercent}%`;
            bgVolumeSlider.disabled = !this.settings.backgroundEnabled;
        }
        
        if (bgSpeedSlider && bgSpeedValue) {
            const bgSpeedPercent = Math.round(this.settings.backgroundSpeed * 100);
            bgSpeedSlider.value = bgSpeedPercent;
            bgSpeedValue.textContent = `${this.settings.backgroundSpeed.toFixed(1)}x`;
            bgSpeedSlider.disabled = !this.settings.backgroundEnabled;
        }
        
        if (bgSoundToggle) {
            bgSoundToggle.checked = this.settings.backgroundEnabled;
        }
        ...
        presetButtons.forEach(btn => {
            ...
            if (preset === 'quiet' && this.settings.volume < 0.4) {
                btn.classList.add('active');
            } else if (preset === 'normal' && this.settings.volume >= 0.4 && 
                       this.settings.volume <= 0.7 && this.settings.speed === 1.0) {
                btn.classList.add('active');
    ...
    // Show temporary status message
    showSoundStatus(message, type = 'info') {
        let status = document.getElementById('soundStatus');
        ...
        status.className = `sound-status show ${type}`;
...
window.addEventListener('load', async function() {
    ...
    if (soundToggle) {
        soundToggle.classList.remove('audio-loading');
    ...
    // Auto-start background sound on user interaction
    document.addEventListener('click', async function startBackgroundOnInteraction() {
        if (SoundManager.settings.backgroundEnabled && !SoundManager.isPlaying) {
...
function setupSoundEventListeners() {
    ...
    document.getElementById('bgVolumeSlider')?.addEventListener('input', function() {
        SoundManager.setBackgroundVolume(this.value);
    });
    
    document.getElementById('bgSpeedSlider')?.addEventListener('input', function() {
        SoundManager.setBackgroundSpeed(this.value);
    });
    
    document.getElementById('bgSoundToggle')?.addEventListener('change', function() {
...
// Add these event listeners after your SoundManager initialization code

// Setup typing event listeners (IMPORTANT: Add this function)
...
window.addEventListener('load', async function() {
    ...
    setupSoundEventListeners();
    setupTypingEventListeners(); // ← ADD THIS LINE
